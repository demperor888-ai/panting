      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="tech-panel overflow-hidden">
          <div className="relative z-10 grid md:grid-cols-2 gap-8 p-8">
            {/* 产品图片 */}
            <div className="h-80 bg-dark-100 rounded-xl flex items-center justify-center overflow-hidden shadow-inner">
              <img
                src={product.image}
                alt={product.name}
            ...
            {/* 产品信息 */}
            <div>
              <span className="inline-block rounded-md bg-teal-500/10 px-3 py-1 text-sm font-medium text-primary-700 border border-teal-300/30 mb-4">
                {category?.name}
              </span>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.features.map((feature, index) => (
                    <span key={index} className="rounded-lg bg-teal-500/8 px-4 py-2 text-primary-700 text-sm border border-teal-200/30">
                      {feature}
                    </span>
                  ))}
                </div>
              ...
              <Link href="/contact" className="glow-btn px-8 py-3 inline-flex items-center gap-2 rounded-lg">
                咨询报价
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-12">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-2xl font-bold text-dark-800">相关产品</h2>
            </div>
            <div className="grid md:grid-cols-3 gap-6">
              {relatedProducts.map((p) => (
                <Link key={p.id} href={`/products/${p.id}`} className="tech-panel block overflow-hidden group">
                  <div className="relative z-10">
                    <div className="h-40 bg-dark-100 flex items-center justify-center overflow-hidden rounded-lg">
                      <img
                        src={p.image}
                        alt={p.name}
